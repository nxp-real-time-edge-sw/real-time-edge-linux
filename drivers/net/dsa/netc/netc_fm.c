...
static struct pci_dev *netc_switch_fm_pdev;
...
static u32 netc_fm_set_ipft(struct netc_fm_priv *priv, u32 ist_eid, struct netc_fm_conf *config)
{
	...
	keye->ethertype = cpu_to_be16(config->filter.ethertype);
	...
	ret = ntmp_ipft_add_entry(&priv->netc_switch->user, ipft);
	if (ret)
		return NTMP_NULL_ENTRY_ID;

	return ipft->entry_id;
}

static u32 netc_fm_alloc_ist(struct ntmp_user *user)
{
	u32 entry_id;

	entry_id = ntmp_lookup_free_eid(user->ist_eid_bitmap, user->caps.ist_num_entries);
	return entry_id;
}

static void netc_fm_free_ist(struct ntmp_user *user, u32 entry_id)
{
	if (entry_id == NTMP_NULL_ENTRY_ID)
		return;

	ntmp_clear_eid_bitmap(user->ist_eid_bitmap, entry_id);
}

static u32 netc_fm_set_ist(struct netc_fm_priv *priv, u32 ett_eid, struct netc_fm_conf *config)
{
	struct ntmp_user *user = &priv->netc_switch->user;
	...
	ist_eid = netc_fm_alloc_ist(user);
	if (ist_eid == NTMP_NULL_ENTRY_ID)
		return NTMP_NULL_ENTRY_ID;

	ist_entry->entry_id = ist_eid;
	ret = ntmp_ist_add_entry(&priv->netc_switch->user, ist_entry);
	if (ret)
		goto err;

	return ist_eid;
err:
	netc_fm_free_ist(user, ist_eid);
	return NTMP_NULL_ENTRY_ID;
}

static u32 netc_fm_alloc_ett(struct ntmp_user *user)
{
	u32 entry_id;

	entry_id = ntmp_lookup_free_eid(user->ett_gid_bitmap, user->ett_bitmap_size);
	return entry_id;
}

static void netc_fm_free_ett(struct ntmp_user *user, u32 entry_id)
{
	if (entry_id == NTMP_NULL_ENTRY_ID)
		return;

	ntmp_clear_eid_bitmap(user->ett_gid_bitmap, entry_id);
}
...
static int netc_fm_update_ett(struct netc_fm_priv *priv)
{
	...
	ret = ntmp_ett_add_or_update_entry(&priv->netc_switch->user, priv->ett_eid, false, ett_cfg);
	if (ret)
		return ret;
...
static u32 netc_fm_set_ett(struct netc_fm_priv *priv, u32 fmt_eid, struct netc_fm_conf *config)
{
	...
	ett_eid = netc_fm_alloc_ett(user);
	if (ett_eid == NTMP_NULL_ENTRY_ID)
		goto err;

	ret = ntmp_ett_add_or_update_entry(&priv->netc_switch->user, ett_eid, true, ett_cfg);
	if (ret) {
		netc_fm_free_ett(user, ett_eid);
		goto err;
	}
	return ett_eid;
...
static u32 netc_fm_alloc_fmt(struct ntmp_user *user)
{
	u32 entry_id;

	entry_id = ntmp_lookup_free_eid(user->fmt_eid_bitmap, user->caps.fmt_num_entries);
	return entry_id;
}

static void netc_fm_free_fmt(struct ntmp_user *user, u32 entry_id)
{
	if (entry_id == NTMP_NULL_ENTRY_ID)
		return;

	ntmp_clear_eid_bitmap(user->fmt_eid_bitmap, entry_id);
}
...
static u32 netc_fm_set_fmt(struct netc_fm_priv *priv, u32 fmdt_eid, struct netc_fm_conf *config)
{
	...
	if (!priv->datasetmsg_cnt || priv->datasetmsg_cnt > NETC_FM_MAX_DATASETMSG)
		return NTMP_NULL_ENTRY_ID;
	...
	fmt_eid = netc_fm_alloc_fmt(user);
	if (fmt_eid == NTMP_NULL_ENTRY_ID)
		goto err;

	ret = ntmp_fmt_add_or_update_entry(&priv->netc_switch->user, fmt_eid, true, fmt_cfg);
	if (ret) {
		netc_fm_free_fmt(user, fmt_eid);
		goto err;
	}
	return fmt_eid;
...
static u32 netc_fm_alloc_fmdt(struct ntmp_user *user, u32 size)
{
	u32 blocks = DIV_ROUND_UP(size, NETC_FM_FMDT_BLOCK_UNIT);
	u32 entry_id;

	entry_id = ntmp_lookup_free_words(user->fmdt_eid_bitmap, user->caps.fmdt_num_blocks,
					  blocks);
	return entry_id;
}

static void netc_fm_free_fmdt(struct ntmp_user *user, u32 entry_id, u32 size)
{
	u32 blocks = DIV_ROUND_UP(size, NETC_FM_FMDT_BLOCK_UNIT);
	...
	ntmp_clear_words_bitmap(user->fmdt_eid_bitmap, entry_id, blocks);
}
...
static void netc_fm_cleanup(struct netc_fm_priv *priv)
{
	...
	if (priv->ipft_eid != NTMP_NULL_ENTRY_ID)
		ntmp_ipft_delete_entry(&priv->netc_switch->user, priv->ipft_eid);

	priv->ipft_eid = NTMP_NULL_ENTRY_ID;

	if (priv->ist_eid != NTMP_NULL_ENTRY_ID) {
		ntmp_ist_delete_entry(&priv->netc_switch->user, priv->ist_eid);
		netc_fm_free_ist(user, priv->ist_eid);
	}
	priv->ist_eid = NTMP_NULL_ENTRY_ID;

	if (priv->ett_eid != NTMP_NULL_ENTRY_ID) {
		ntmp_ett_delete_entry(&priv->netc_switch->user, priv->ett_eid);
		netc_fm_free_ett(user, priv->ett_eid);
	}
	priv->ett_eid = NTMP_NULL_ENTRY_ID;

	if (priv->fmt_eid != NTMP_NULL_ENTRY_ID) {
		ntmp_fmt_delete_entry(&priv->netc_switch->user, priv->fmt_eid);
		netc_fm_free_fmt(user, priv->fmt_eid);
	}
	priv->fmt_eid = NTMP_NULL_ENTRY_ID;

	if (priv->fmdt_eid != NTMP_NULL_ENTRY_ID)
		netc_fm_free_fmdt(user, priv->fmdt_eid, priv->fmdt_size);
				    ...
				    size_t count, loff_t *ppos)
{
	...
	ntmp_fmt_delete_entry(&priv->netc_switch->user, old_fmt_eid);
	netc_fm_free_fmt(user, old_fmt_eid);
	netc_fm_free_fmdt(user, old_fmdt_eid, old_fmdt_size);
...
err_free_fmt:
	ntmp_fmt_delete_entry(&priv->netc_switch->user, new_fmt_eid);
	netc_fm_free_fmt(user, new_fmt_eid);
	priv->fmt_eid = old_fmt_eid;
err_free_fmdt:
	netc_fm_free_fmdt(user, new_fmdt_eid, priv->fmdt_size);
	priv->fmdt_eid = old_fmdt_eid;
err_restore:
	priv->fmdt_size = old_fmdt_size;
	mutex_unlock(&netc_switch_fm_mutex);
	return ret;
err:
	mutex_unlock(&netc_switch_fm_mutex);
...
static void __exit netc_switch_fm_driver_exit(void)
{
	...
	pci_dev_put(netc_switch_fm_pdev);
	netc_switch_fm_pdev = NULL;
	netc_switch_g = NULL;
}
