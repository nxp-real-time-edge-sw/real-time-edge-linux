...
int hms_hwtstamp_set(struct dsa_switch *ds, int port,
...
int hms_hwtstamp_get(struct dsa_switch *ds, int port,
		     struct kernel_hwtstamp_config *config)
{
	struct hms_private *priv = ds->priv;
			     ...
			     struct hms_tx_ts_desc *desc)
{
	...
	if (skb_match->sk &&
	    READ_ONCE(skb_match->sk->sk_tsflags) & SOF_TIMESTAMPING_BIND_PHC)
