...
l4ip6:
	...
	ntmp_ipft_add_entry(&si->ntmp_user, entry);
	return 0;
}
...
static int enetc4_configure_rxnfc(struct net_device *ndev, struct ethtool_rxnfc *rxnfc)
{
	...
	case ETHTOOL_SRXCLSRLINS:
		...
		enetc4_set_ipft_entry(si, &rxnfc->fs, &entry_id);
			  ...
			  struct netlink_ext_ack *extack)
{
	...
	if (rxfh->key) {
        ...
        ecat_enetc_set_rss_key(si, rxfh->key);
    }
			       ...
			       struct netlink_ext_ack *extack)
{
	...
	if (kernel_param->rx_buf_len &&
		...
		err = ecat_enetc_reconfigure(priv, extended,
