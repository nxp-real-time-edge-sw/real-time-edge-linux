...
static void rpmsg_eptdev_release_device(struct device *dev)
{
	...
	ida_free(&rpmsg_perf_minor_ida, MINOR(eptdev->dev.devt));
	kfree(eptdev);
}
				 ...
				 struct rpmsg_channel_info chinfo)
{
	...
	ret = ida_alloc_max(&rpmsg_perf_minor_ida, RPMSG_DEV_MAX - 1, GFP_KERNEL);
	if (ret < 0)
		goto free_eptdev;
...
free_minor_ida:
	ida_free(&rpmsg_perf_minor_ida, MINOR(dev->devt));
free_eptdev:
	put_device(dev);
	kfree(eptdev);
...
MODULE_DESCRIPTION("RPMSG performance test driver");
MODULE_AUTHOR("Hou Zhiqiang <Zhiqiang.Hou@nxp.com>");
